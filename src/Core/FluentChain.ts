...
export class FluentChain {
    ...
    constructor(
      ...
      private pluginConfigs: Array<{ name: string; options?: Partial<AudioPluginOptions> }>,
    ...
    private buildChain(): void {
      this.transforms = this.pluginConfigs.map(({ name, options }) => {
        if (!this.registry.has(name)) throw new Error(`Plugin not found: ${name}`);
        const mergedOptions: Required<AudioPluginOptions> = { ...this.defaultOptions, ...options };
