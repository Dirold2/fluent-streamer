...
export default class PluginHotSwap extends Transform {
   ...
   */
  private attach(chain: Transform): void {
    chain.on("data", (chunk) => {
      if (!this.push(chunk)) chain.pause();
    });
    chain.once("error", (err) => this.emit("error", err));
    chain.once("end", () => {
      if (!this.swapping) this.push(null);
    });
  }
   ...
   */
  private detach(chain: Transform): void {
   ...
   */
  private performHardSwap(newChain: Transform): void {
    const old = this.current;
    this.detach(old);
    old.destroy();
   ...
   * @returns {boolean}
   */
  private areCompatible(a: any, b: any): boolean {
    return (
   ...
   */
  private copyState(target: any, source: any): void {
    if (
      typeof source.getOptions === "function" &&
